...
```json
{ "enabled": true, "disclosedAt": "...", "activationSentAt": "..." }
```

It contains no personal data — just a flag and timestamps — and can be deleted at any
time (doing so also resets the one-time-ping state and re-triggers disclosure on next
use).
